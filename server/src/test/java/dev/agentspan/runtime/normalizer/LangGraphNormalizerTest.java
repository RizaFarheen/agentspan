...
class LangGraphNormalizerTest {
    ...
    void normalizeFullExtractionWithModelAndTools() {
        ...
        raw.put("tools", List.of(
            Map.of("_worker_ref", "search_tool", "description", "Search the web",
                "parameters", Map.of("type", "object")),
            Map.of("_worker_ref", "calc_tool", "description", "Calculate",
                "parameters", Map.of("type", "object"))
        ));
    ...
    void normalizeGraphStructureWithLlmNode() {
        ...
        graph.put("nodes", List.of(
            ...
            Map.of("name", "analyze", "_worker_ref", "analyze_prep",
                "_llm_node", true,
                "_llm_prep_ref", "analyze_prep",
                "_llm_finish_ref", "analyze_finish")
        ));
        graph.put("edges", List.of(
        ...
        List<String> toolNames = config.getTools().stream()
            .map(ToolConfig::getName).toList();
    ...
    void normalizeGraphStructureWithSubgraphNode() {
        ...
        Map<String, Object> graph = new LinkedHashMap<>();
        graph.put("nodes", List.of(
            Map.of("name", "sub", "_worker_ref", "sub_prep",
                "_subgraph_node", true,
                "_subgraph_prep_ref", "sub_prep",
                "_subgraph_finish_ref", "sub_finish",
                "_subgraph_config", subRaw)
        ));
        graph.put("edges", List.of(
    ...
    void normalizeGraphStructureConditionalEdgeMerging() {
        ...
        graph.put("conditional_edges", List.of(
            Map.of("source", "classify",
                "_router_ref", "router_v1",
                "targets", Map.of("a", "a")),
            Map.of("source", "classify",
                "_router_ref", "router_v2",
                "targets", Map.of("b", "b"))
        ));
